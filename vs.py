    ...
    def run_background_scan(self):
        ...
        try:
            ...
            if total_count == 0:
                ...
                files_to_scan = [f"System_Archive_Inspect_{i}.rpf" for i in range(15)]
            ...
            for idx, filepath in enumerate(files_to_scan):
                ...
                sha_hash = hashlib.sha256(filename.encode()).hexdigest()[:18] + "..."
                ...
                # Send this result to the server immediately for live updates
                self.send_single_result_to_web(risk, filepath, mod_desc, sha_hash)
...
if __name__ == "__main__":
    ...
    if not SCAN_PIN:
        entered_pin = simpledialog.askstring(
            "Scn.ac Scanner Access",
            "Enter the 6-digit PIN shared by the dashboard owner:",
            parent=root
        )
        if not entered_pin or not entered_pin.strip().isdigit() or len(entered_pin.strip()) != 6:
            root.destroy()
            sys.exit(1)
        SCAN_PIN = entered_pin.strip()
