...
def scan_progress():
    ...
    visible_files = app_state["scanned_files"] if app_state.get("scan_owner") == owner else []
    return jsonify({
        "is_scanning": app_state["is_scanning"],
...
def add_scan_result():
    ...
    data = request.get_json() or {}
    owner = current_user() or str(data.get("user", "Unknown User")).strip() or "Unknown User"
    app_state["scan_owner"] = owner
...
def generate_pin():
    ...
    if any(record_owner(entry) == owner for entry in app_state["generated_pins"]):
        return jsonify({"status": "error", "message": "This account already has a PIN. Ask the administrator to delete it before creating another."}), 409
...
def dashboard_stats():
    ...
    if error_response:
        return error_response
    files = app_state["scanned_files"] if app_state.get("scan_owner") == owner else []
    return jsonify({
