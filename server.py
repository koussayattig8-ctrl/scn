...
def list_access_keys():
    if request.headers.get("X-Admin-Username") != ADMIN_USERNAME or request.headers.get("X-Admin-Password") != ADMIN_PASSWORD:
        return jsonify({"status": "error", "message": "Admin credentials are invalid."}), 403
    return jsonify({
        "status": "success",
        "keys": [{
...
def login_with_access_key():
    ...
    with STATE_LOCK:
        ...
        if key_record.get("used"):
            return jsonify({"status": "used", "message": "This access key has already been used."}), 409
    ...
    return jsonify({
        "status": "success",
        "message": "Login accepted. This key cannot be used again.",
        "username": username,
        "expires_at": key_record.get("expires_at"),
