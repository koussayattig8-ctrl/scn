...
SCANNER_EXE_PATH = os.path.join(os.path.dirname(__file__), "dist", "SentinelScanner.exe")
...
def download_scanner_file():
    if not os.path.isfile(SCANNER_EXE_PATH):
        return jsonify({"status": "error", "message": "Scanner download is not available yet."}), 404
    return send_file(SCANNER_EXE_PATH, as_attachment=True, download_name='SentinelScanner.exe')
